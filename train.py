...
def train(train_directories, epoch):
    ...
    final_lr = 1e-5
    decay = (learning_rate - final_lr) / epoch
    ...
    for epoch in range(epoch):
        ...
        if epoch % 1 == 0:
            ...
            for _, val_data in enumerate(loaded_valid_data):
                ...
                cur_mse, cur_ssim, cur_psnr = eval(gt.data.cpu().numpy(), sr.data.cpu().numpy())
                total_mse += cur_mse / len(loaded_valid_data)
                total_ssim += cur_ssim / len(loaded_valid_data)
                total_psnr += cur_psnr / len(loaded_valid_data)
